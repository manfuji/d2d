...
const EditProducts = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (selectedProduct !== {}) {
      // const finalData = Object.assign({}, selectedProduct, formData);
      const body = {
        name: formData.name !== "" ? formData.name : selectedProduct.name,
        price:
        ...
        shop: userId,
        category:
          Number(formData.category) !== 0
            ? Number(formData.category)
            : selectedProduct.category,
      };
      console.log("======>poooop", body);
      // console.log(body);

      axios
        .put(
          process.env.REACT_APP_BASE_URL +
            `/shop/product/${selectedProduct.id}/`,
          body,
          config
        )
        .then((response) => {
  ...
  const HandleUpdate = (id) => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/shop/product/${id}`)
      .then((response) => {
